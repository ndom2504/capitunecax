...
export const GET: APIRoute = async ({ request, cookies, redirect, locals }) => {
  ...
  try {
    ...
    cookies.set('capitune_user', JSON.stringify({ name, email, picture: profile.picture, provider: 'google', role: ADMIN_EMAILS.includes(email) ? 'admin' : 'user' }), {
      path: '/',
      httpOnly: false,
      secure: isHttps,
      sameSite: 'lax',
