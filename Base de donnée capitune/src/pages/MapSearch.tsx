...
import { GraduationCap, Briefcase, Home, Filter, Map as MapIcon, List } from 'lucide-react';
import { ALL_SCHOOLS, ALL_JOBS, ALL_HOUSING } from '@/data/mockData';
import 'leaflet/dist/leaflet.css';
import L from 'leaflet';
import { renderToStaticMarkup } from 'react-dom/server';

// Fix for default Leaflet icons in Vite/Webpack
// We use custom DivIcons instead to avoid asset path issues entirely and allow styling
...
export default function MapSearch() {
  ...
  const currentData = useMemo(() => {
    switch (activeTab) {
      case 'education': return ALL_SCHOOLS;
      case 'job': return ALL_JOBS;
      case 'housing': return ALL_HOUSING;
      default: return [];
    }
  }, [activeTab]);
        ...
        <div className="w-full md:w-64 bg-surface border-b md:border-b-0 md:border-r border-border p-4 z-10 flex flex-col gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 rounded-lg border border-border bg-bg-light text-text focus:outline-none focus:ring-2 focus:ring-primary/20 text-sm"
              value={searchTerm}
